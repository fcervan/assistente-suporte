    ...
    pts = [
        PointStruct(id=str(uuid.uuid4()), vector=v,
                    payload={"texto": c["texto"], "fonte": c.get("fonte", ""),
                              "pagina": c.get("pagina"), "secao": c.get("secao", ""),
                              "chunk_index": c.get("chunk_index"),
                              "doc_id": c.get("doc_id", ""),
                              "versao_chunk": c.get("versao_chunk",
                                                    config.VERSAO_CHUNK)})
...
def buscar(query: str, top_k: int = 8) -> list[dict]:
    ...
    hits = client().query_points(
        collection_name=config.QDRANT_COLLECTION, query=qv, limit=pool
    ).points
    densos = [{"texto": h.payload.get("texto", ""), "fonte": h.payload.get("fonte", ""),
                "pagina": h.payload.get("pagina"), "secao": h.payload.get("secao", ""),
