...
threads estilo ChatGPT (criar/continuar/renomear/apagar) e **eval dourado 15/15**.

## Demonstração

Eval dourado sobre o Manual da API SmartLabel (`tests/douradas.csv`): 15 perguntas
reais (autenticação, método Registrar, responses 200/206/400/401, impressão de
etiquetas, envio em lote…) — **hit@4 em 15/15** após o chunking v2 (antes: 10/15,
...
**Ingestão:** na aba *Enviar docs* (admin) ou `POST /ingest` — PDF/DOCX/PPTX/XLSX/CSV/
HTML/XML/TXT/MD. Reimportação total via `docker compose exec api python -m src.reingest_v2`.

**Primeiro acesso:** registre-se com `role=admin` (`POST /auth/register`) e envie o
...
├── api/src/
│   ├── main.py            # FastAPI: auth JWT + chat/threads + ingest + tickets
...
│   ├── duckdb_store.py    # users, threads (título+resumo), interacoes
...
│   └── auth.py            # JWT + hash bcrypt
├── web/src/               # React 18 + Vite (Chat c/ sidebar, Ingest, Admin, Login)
├── tests/                 # 73 testes + CSV dourado (15 perguntas)
└── docker-compose.yml     # api :8002 + web :5175 + qdrant :6333
```

## Qualidade

* **73 testes automatizados** (cobrem API, grafo, chunking, RRF, threads, auth); dependências
...
multi-LLM, frontend React, suite de 73 testes e CI.
